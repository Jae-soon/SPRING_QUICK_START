...
**프로젝트 시작**
```
1. File -> New -> Other -> spring -> Spring Legacy Project -> Spring MVC Project 클릭 및 프로젝트명 입력
2. com.mycompany.myapp* 과 같이 자신의 도메인을 입력해주도록 하자 (디렉토리 설정)
3. 프로젝트 생성됨
```
**xml 설정 파일 생성**
```
1. src.main.webapp.resources 마우스 오른쪽 클릭 -> new -> other -> Spring의 Spring Bean Configuration File 클릭
2. 파일 이름 : applicationContext 그리고 쭉 next 해서 생성하자
```
...
```class="속성값"```은 패키지 경로를 포함된 전체 클래스 경로를 지정해야 한다.  
```
클래스 이름을 어느정도 기술 후 <Ctrl>+<Space>를 눌러서 자동 완성 기능을 사용할 수 있다.
```
## 1.1. 스프링 컨테이너 구동 및 테스트
```
...
public class TVUser {
	...
	public static void main(String[] args) {
		...
		TV tv = (TV)factory.getBean("tv"); // <bead id="tv" class="~">를 호출하여 해당 객체를 얻는다.    
		tv.powerOn();
		tv.volumeUp();
		tv.volumeDown();
		tv.powerOff();
...
3. getBean() 메소드로 <Bean>의 id="tv"인 객체를 요청(Lookup)
4. id="tv"는 class="polymorphism.SamsungTV"의미하므로 해당 객체 즉, SamsungTV 객체를 반환한다.     
  
이제 tv 객체를 ```LgTV```로 변경하고자 한다면 ```applicationContext.xml```파일을 수정 해주면 된다.    
...
트랜잭션 관리나 메시지 기반의 다국어 처리 등 다양한 기능을 지원하고 컨테이너식으로 동작한다.            
웹 어플리케이션 개발도 지원하므로 대부분 스프링 프로젝트는 ApplicationContext 유형의 컨테이너를 이용한다.        
...
하지만 아직 배우는 단계이므로 ```GenericXmlApplicationContext```위주로 사용할 예정이다.     
...
즉, ```<import>``` 태그를 이용하여 여러 스프링 설정 파일을 포함함으로써 한 파일에 작성하는 것과 같은 효과를 낼 수 있다.    
...
물론 ```name```속성값 역시 유일해야한다.(언뜻보면 ```name```속성이 편해 보이지만 어느정도 규칙이 있는 ```id```를 사용한다.)       
...
놀라운 점은 계속 저장하지 않고 조금 더 메모리적으로 효율성을 높이기 위해 삭제한다는 것이다.    
**applicationContext.xml**
```
<?xml version="1.0" encoding="UTF-8"?>
<beans xmlns="http://www.springframework.org/schema/beans"
...
```true/false``` 값이 들어오며 이를 이용해 메모리 관리를 더 효울적으로 할 수 있게 된다.       
```
...
```
## 3.4. scope 속성
프로그램을 개발하다 보면 수많은 객체들이 생성되고 그중에는 하나만 생성되어도 상관없는 객체들이 있다.     
...
**```scope 속성값```은 디폴트 값은 싱글톤이다.** 즉, 해당 bean이 스프링 컨테이너에 의해 단 하나만 생성되어 운용 되도록 한다.         
...
객체 삭제 전에 처리할 로직 처리...
```
싱글톤이 적용 되어 있으므로 INFO 는 한번 호출한다. 즉, 하나의 bean 객체만이 존재한다.          
